...
describe "Shopping Cart Rack App" do
  ...
  it "Returns 404 for a bad route which includes the word items, but is not exclusively items, e.g., /items50/" do
    get '/items50'
    expect(last_response.body).to include("Route not found")
    expect(last_response.status).to be(404)
  end

  it "Returns 404 for a bad route which includes the word items, but is not exclusively items, e.g., /someitems/" do
    get '/50items'
    expect(last_response.body).to include("Route not found")
    expect(last_response.status).to be(404)
  end
